...
- Pass FD 4 of current shell to `ssh` as the connection
- FD 4 must be some open socket, connected to some `ssh`-deamon, of course.
- As `ssh` closes all FDs from the outside, this needs some temporary socket
...
- connection to some host is not yet implemented
  - Also it is wrongly determined
